...
- **MCP door + cutover (Epic 20)** — the widget POSTs to the delegating webhook `/webhook/expliq-support-agent-v2`, which calls the shared `file_support_request` sub-workflow (same brain as the MCP door at `/mcp/expliq-support-mcp`). `N8N_SUPPORT_WEBHOOK_URL` (Production + Preview) holds the v2 path; its secret is shared with the n8n Header-Auth credential `Q9PLFpkotcPFTRRe`. **Caution:** both `N8N_SUPPORT_WEBHOOK_*` vars are **Sensitive** in Vercel, so `vercel env pull` returns them empty. On 2026-05-29 the Doppler migration imported them empty (a value-diff of empty-vs-empty passed pre-flight) and broke the prod widget ("Support service is not configured"). After any secrets migration touching these, verify the widget actually answers; do not rely on a value-diff. The old monolith `/webhook/expliq-support-agent` (`B0YWkBWQa9NEfX9r`) is the now-deactivated rollback.
