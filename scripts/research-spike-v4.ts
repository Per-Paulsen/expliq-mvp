/**
 * Research Spike v4 — Exhaustive reasoning via two-phase methods
 *
 * Builds on v3 (rubric, detectability, structural features, method-based).
 * Adds: enumerate-then-evaluate (T1), state machine framing (T4),
 * completeness verification (T3), taxonomy pre-extraction (T6).
 *
 * Usage: same as v3 but script name differs
 *   npx tsx scripts/research-spike-v4.ts --model anthropic/claude-opus-4
 *   npx tsx scripts/research-spike-v4.ts --model anthropic/claude-sonnet-4
 */
...
const RESULTS_DIR = path.join(process.cwd(), "specs/research-spike-results/v4");
...
const ALL_WORKFLOWS_FLAG = args.includes("--all");
...
// ============================================================
// STRUCTURAL FEATURE EXTRACTION (v3 base + v4 taxonomy extraction)
// ============================================================
...
/** v4: Extract categorical taxonomies from node parameters */
function extractTaxonomies(nodes: any[]): Array<{ source: string; name: string; values: string[] }> {
  ...
  for (const node of nodes) {
    // Structured output parsers with JSON schemas containing enums
    ...
    // Switch/router nodes with rule conditions
    if (node.type?.includes("switch") || node.type?.includes("router")) {
      const rules = node.parameters?.rules?.values || node.parameters?.rules || [];
      if (Array.isArray(rules) && rules.length > 0) {
        const values = rules
          .map((r: any) => r.value || r.outputKey || r.label)
          .filter(Boolean);
        if (values.length > 1) {
          taxonomies.push({
            source: `${node.name} (switch/router)`,
    ...
    // AI prompt nodes — extract category lists from prompt text
    if (node.parameters?.messages?.messageValues) {
      for (const msg of node.parameters.messages.messageValues) {
        if (typeof msg.message === "string" && msg.message.length > 200) {
          // Look for category headers (## Category or **Category**)
          const categoryMatches = msg.message.match(/\*\*([A-Z][^*]+)\*\*\s*—/g);
          if (categoryMatches && categoryMatches.length >= 3) {
            const categories = categoryMatches.map((m: string) =>
              m.replace(/\*\*/g, "").replace(/\s*—.*/, "").trim()
            );
            taxonomies.push({
              source: `${node.name} (AI prompt categories)`,
...
function extractStructuralFeatures(wf: any) {
  ...
  const triggerNodes = functionalNodes.filter((n: any) => n.type.toLowerCase().includes("trigger"));
  ...
  // v4: Extract taxonomies
  const taxonomies = extractTaxonomies(functionalNodes);
...
// Per-automation: unchanged from v3 (already lean and effective)
...
// Call 1: v4 — two-phase methods, state machine, completeness verification
...
<instructions>
You receive per-workflow summaries, execution statistics, instance metadata, and pre-extracted taxonomies (categorical lists found in the automation data). Apply these five analytical methods systematically. Each method has two phases: first ENUMERATE completely, then ANALYZE each item.

1. CAPABILITY ENUMERATION:
   Phase A — CENSUS: List every distinct business function served by automations in this collection.
   Phase B — ANALYSIS: For each function, list all automations that serve it. When multiple serve the same function, assess whether intentional or drift.

2. DEPENDENCY GRAPH CONSTRUCTION:
   Phase A — CENSUS: List every external system and credential referenced across all automations.
   Phase B — ANALYSIS: For each system, count dependent processes. Assess blast radius if unavailable. Flag systems touched by >50% of workflows.

3. DOMAIN-COVERAGE VERIFICATION:
   Phase A — CENSUS: Extract a complete, deduplicated list of every business domain referenced in the automation data. Sources to scan exhaustively:
   - Every category in the pre-extracted taxonomies (if provided)
   - Every CTA, link text, or action label in email templates or notifications
   - Every sheet name, column header, or data field name
   - Every business term in workflow names, descriptions, or AI prompts
   Number each domain in the list.
   Phase B — COVERAGE CHECK: For each numbered domain, state whether a corresponding automation exists (cite the workflow) or does not exist (mark as GAP). Every item must receive a verdict.

4. OPERATIONAL COHERENCE CHECK:
   Phase A — CENSUS: List every automation with its status (active/inactive), execution count, error rate, and assessed business importance.
   Phase B — ANALYSIS: For each, flag contradictions between status, execution patterns, and importance.

5. LIFECYCLE COMPLETENESS TRACE:
   Phase A — PARTICIPANT CENSUS: For each business process, identify ALL participants and stakeholders — including those who receive outcomes AND those who receive non-outcomes (rejected applicants, unselected candidates, denied requests, unsuccessful participants).
   Phase B — STATE MACHINE: Model each process as a state machine. Define all states a participant can be in. For each state, define ALL possible transitions. For each transition, check whether an automation handles it. A participant who enters the process but reaches no defined terminal state is a SILENT DROP-OFF — a high-priority gap.
...
Group workflows into BUSINESS PROCESSES (end-to-end flows), not technical categories. Cross-cutting workflows belong in "Operations."

COMPLETENESS VERIFICATION (mandatory final step):
After all five methods, self-check:
A. Count total items in each Phase A census. Count verdicts in each Phase B/C. If counts don't match, complete the missing verdicts.
B. For every automated workflow, state the population it serves. Then state the population it does NOT serve. If the non-served population has no automated outcome, add to crossWorkflowFindings.
C. For every taxonomy extracted, confirm every value received a coverage verdict.
...
<output_format>
Return a JSON object with reasoning first. The reasoning MUST show evidence of all five methods being applied with both phases.

{
  "reasoning": "Step-by-step analysis showing Phase A censuses and Phase B analyses for all five methods, plus completeness verification results.",
  "processes": [
    {
  ...
  "systemLandscape": [
    {
      ...
      "workflowCount": 0,
      "narrative": "What usage pattern reveals",
  ...
  "crossWorkflowFindings": [
    {
      ...
      "method": "Which method and phase surfaced this"
    }
  ]
}
</output_format>
...
- Do NOT skip Phase A in any method. The census must be complete before analysis begins.
- Do NOT stop after finding a few patterns. Every census item must receive a verdict.
</anti_patterns>`;

// Call 2: v4 — ensure suggested processes generate recommendations
...
COMPLETENESS: Every gap identified in the landscape analysis (suggested processes, missing lifecycle steps, silent drop-offs, uncovered domains) should be addressed by at least one recommendation. If the landscape identifies a gap, your job is to recommend what to do about it.
...
- Do NOT leave landscape gaps unaddressed. Every identified gap deserves a recommendation.
</anti_patterns>`;
...
function formatStep1Message(wfJson: any, features: ReturnType<typeof extractStructuralFeatures>, stats: ReturnType<typeof computeExecutionStats>) {
...
function parseJSON(raw: string, fallbackPath: string): any {
  ...
  try {
    ...
    fs.writeFileSync(fallbackPath, raw);
    throw new Error(`Could not extract JSON. Raw saved to ${fallbackPath}`);
  }
}
...
async function main() {
  ...
  console.log(`Workflows: ${ALL_WORKFLOWS_FLAG || STEP === 0 ? "all 8" : "3 test"}\n`);
  ...
  if (STEP === 0 || STEP === 1) {
    const workflows = (ALL_WORKFLOWS_FLAG || STEP === 0) ? ALL_WORKFLOW_LIST : TEST_WORKFLOWS;
    ...
    for (const wf of workflows) {
      ...
      const taxInfo = features.taxonomies.length > 0
        ? ` | ${features.taxonomies.length} taxonomies`
        : "";
      console.log(`  ${wf.label}: ${features.nodeCount} nodes, ${features.systemsDetected.join("+")||"none"}, ${stats.total} execs${taxInfo}`);

      const start = Date.now();
      const response = await openai.chat.completions.create({
        model: MODEL,
        messages: [
          { role: "system", content: PER_AUTOMATION_PROMPT },
          { role: "user", content: formatStep1Message(wfJson, features, stats) },
        ],
      ...
      const parsed = parseJSON(raw, path.join(RESULTS_DIR, `step1-${wf.label}-raw.txt`));
  ...
  if (STEP === 0 || STEP === 2) {
    ...
    // Collect tags, credentials, and ALL taxonomies across workflows
    const tags = new Set<string>();
    const creds = new Set<string>();
    const allTaxonomies: Array<{ source: string; name: string; values: string[] }> = [];
    for (const wf of ALL_WORKFLOW_LIST) {
      ...
      // Extract taxonomies from each workflow
      const features = extractStructuralFeatures(wfJson);
      allTaxonomies.push(...features.taxonomies);
    }

    // Deduplicate taxonomies by name+values
    const uniqueTaxonomies = allTaxonomies.filter((t, i) =>
      allTaxonomies.findIndex(u => u.name === t.name && JSON.stringify(u.values) === JSON.stringify(t.values)) === i
    );

    const taxonomySection = uniqueTaxonomies.length > 0
      ? `\n<extracted_taxonomies>\nThe following categorical lists were extracted programmatically from the automation data. Each represents a business-defined partition. Your domain-coverage verification must produce a verdict for EVERY value in EVERY taxonomy.\n\n${uniqueTaxonomies.map((t, i) => `${i + 1}. ${t.source}: ${t.values.join(", ")} (${t.values.length} values)`).join("\n")}\n</extracted_taxonomies>`
      : "";
...
</instance_metadata>${taxonomySection}
...
Analyze this automation landscape. Return the JSON object.`;
    ...
    const parsed = parseJSON(raw, path.join(RESULTS_DIR, "step2-raw.txt"));
...
Produce recommendations, synthesis, and visibility expansions. Return the JSON object.`;
    ...
    const parsed = parseJSON(raw, path.join(RESULTS_DIR, "step3-raw.txt"));
    console.log(`  ${elapsed}s | ${response.usage?.prompt_tokens}/${response.usage?.completion_tokens} tok`);
